...
public class ExactlyOneSolutionConfigTest {
	...
	public void testCalcBorderline1() {
		...
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.GREEN, Pictures.BLUE, Pictures.GREEN));
		List<Condition> borderline = new ExactlyOneSolutionConfig().calcBorderline(field);
	...
	public void testCalcBorderline2() {
		...
		CardCreator cc = new CardCreator();
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.GREEN, Pictures.BLUE, Pictures.GREEN));
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.BLUE, Pictures.GREEN, Pictures.RED));
		List<Condition> borderline = new ExactlyOneSolutionConfig().calcBorderline(field);
		assertEquals(2, borderline.size());
	...
	public void testCalcBorderline3() {
		...
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.GREEN, Pictures.BLUE, Pictures.GREEN));
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.BLUE, Pictures.GREEN, Pictures.RED));
		field = field.addedIfFits(cc.createCard(Pictures.GREEN, Pictures.RED, Pictures.RED, Pictures.BLUE));
		List<Condition> borderline = new ExactlyOneSolutionConfig().calcBorderline(field);
		assertEquals(2, borderline.size());
	...
	public void reallyOnlyOneSolution() {
		...
		assertEquals(noSolution.size() + " out of " + solutions.size() + " with no solutions: " + noSolution, 0,
				noSolution.size());
		assertEquals(moreSolutions.size() + " failures out of " + solutions.size() + " solutions: " + moreSolutions, 0,
				moreSolutions.size());
